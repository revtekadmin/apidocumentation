...
if [ "$SWAGGER_JSON_URL" ]; then
  sed -i "s|https://access.theiot.company/version-test/api/1.1/meta/swagger.json|$SWAGGER_JSON_URL|g" $INITIALIZER_SCRIPT
...
if [[ -f "$SWAGGER_JSON" ]]; then
  ...
  sed -i "s|https://access.theiot.company/version-test/api/1.1/meta/swagger.json|$REL_PATH|g" $INITIALIZER_SCRIPT
